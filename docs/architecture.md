...
- Baseline queries (`fetch_step_count_baseline`, `fetch_token_growth_baseline`, etc.) are scoped by `(org_id, agent_id, agent_version)` — `org_id` is required, since `agent_id`/`agent_version` are not guaranteed unique across orgs. **Every join into `events` inside those queries carries `org_id` too.** `run_id` is caller-supplied, so joining on it alone read across tenants: another org's `run.completed` satisfied the completed-run filter for this org's errored run, and their events contributed steps, tokens and durations to this org's baseline. They read `run_baseline_metrics` — durable per-run scalars written at detection time from `RunState` — so a baseline outlives the `events` rows it was derived from and cannot drift from the detector it feeds. The legacy events-derived queries (`_legacy_*_baseline`) remain as a transitional fallback for deployments whose metrics table hasn't filled yet; they share one run-selection CTE (`_RECENT_CLEAN_RUNS_CTE`), which admits only completed runs that fired no live signal — a constant rather than six copies because the copies had already drifted.
