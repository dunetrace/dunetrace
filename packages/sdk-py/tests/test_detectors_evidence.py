...
class TestToolLoopEvidence(unittest.TestCase):
    ...
    def _firing_state(self, args_list=None, successes=None):
        calls = []
        for i in range(5):
            a = args_list[i] if args_list else "same"
            s = successes[i] if successes else None
            calls.append(make_tool_call("web_search", step=i, args=a, success=s))
        state = make_state()
    ...
    def test_args_identical_true_when_same_args(self):
        state = self._firing_state(args_list=["aaa"] * 5)
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["args_identical"] is True

    def test_args_identical_false_when_different_args(self):
        state = self._firing_state(args_list=["aaa", "bbb", "ccc", "ddd", "eee"])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["args_identical"] is False

    def test_args_similar_true_when_two_unique_args(self):
        state = self._firing_state(args_list=["aaa", "bbb", "aaa", "bbb", "aaa"])
        signal = self.detector.on_run_completion(state)
    ...
    def test_args_in_evidence(self):
        state = self._firing_state(args_list=["x", "y", "x", "y", "x"])
        signal = self.detector.on_run_completion(state)
        assert "args" in signal.evidence
        assert len(signal.evidence["args"]) == 5
...
class TestRetryStormDetector(unittest.TestCase):
    ...
    def _failing_streak(self, tool: str, count: int, args_list=None, errors=None) -> RunState:
        state = make_state()
        calls = []
        for i in range(count):
            a = args_list[i] if args_list else f"arg{i}"
            e = errors[i] if errors else None
            calls.append(make_tool_call(tool, step=i + 1, success=False, args=a, error=e))
        state.tool_calls = calls
    ...
    def test_errors_in_evidence(self):
        state = self._failing_streak("api", 3, errors=["err1", "err1", "err1"])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["errors"] == ["err1", "err1", "err1"]

    def test_reason_identical_true_when_same_error(self):
        state = self._failing_streak("api", 3, errors=["e1", "e1", "e1"])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["reason_identical"] is True
        assert signal.evidence["failure_reason"] == "e1"

    def test_reason_identical_false_when_different_errors(self):
        state = self._failing_streak("api", 3, errors=["e1", "e2", "e3"])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["reason_identical"] is False
        assert signal.evidence["failure_reason"] is None

    def test_reason_identical_false_when_errors_are_none(self):
        state = self._failing_streak("api", 3, errors=[None, None, None])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["reason_identical"] is False

    def test_args_identical_true_when_same_args(self):
        state = self._failing_streak("api", 3, args_list=["same"] * 3)
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["args_identical"] is True

    def test_args_identical_false_when_different_args(self):
        state = self._failing_streak("api", 3, args_list=["a", "b", "c"])
        signal = self.detector.on_run_completion(state)
        assert signal.evidence["args_identical"] is False
    ...
    def test_custom_threshold(self):
        ...
        state = self._failing_streak("api", 4)
        assert detector.on_run_completion(state) is None
