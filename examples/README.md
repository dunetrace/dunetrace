...
| Example | What it shows | How to run | Stack or offline |
| --- | --- | --- | --- |
| [`basic_agent.py`](../packages/sdk-py/examples/basic_agent.py) | Manual instrumentation with normal, tool-loop, prompt-injection, and empty-RAG runs. | `pip install dunetrace`<br>`python examples/basic_agent.py` | Needs the backend first: `docker compose up`. |
| [`decorator_agent.py`](../packages/sdk-py/examples/decorator_agent.py) | `@dt.agent()` decorator plus auto-instrumentation, with happy-path runs and failure scenarios. | `pip install dunetrace`<br>`python examples/decorator_agent.py`<br>`SCENARIO=failures python examples/decorator_agent.py` | Needs the backend first: `docker compose up`. |
