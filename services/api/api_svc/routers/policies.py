...
_VALID_TRIGGERS = {
    "tool_call_count", "step_count", "cost_usd",
    "error_count", "finish_reason", "llm_latency_ms", "signal",
}
...
def _validate(condition: ConditionModel, action: ActionModel) -> None:
    if condition.trigger not in _VALID_TRIGGERS:
        raise HTTPException(422, f"Invalid trigger {condition.trigger!r}. Valid: {sorted(_VALID_TRIGGERS)}")
    if condition.operator not in _VALID_OPERATORS:
        raise HTTPException(422, f"Invalid operator {condition.operator!r}. Valid: {sorted(_VALID_OPERATORS)}")
    if action.type not in _VALID_ACTIONS:
        raise HTTPException(422, f"Invalid action type {action.type!r}. Valid: {sorted(_VALID_ACTIONS)}")
...
@router.patch("/{policy_id}/toggle", response_model=Dict[str, Any], summary="Toggle enabled/disabled")
async def toggle(
    policy_id: int,
    _customer: str = Depends(require_customer),
) -> Dict[str, Any]:
