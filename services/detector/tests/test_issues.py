...
class TestAdvanceCleanRuns(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_execute_called_with_agent_id_and_threshold(self):
        pool = _make_pool()
        with patch.object(db_module, "_pool", pool):
            await db_module.advance_clean_runs("agent-1", ["TOOL_LOOP"])
        pool._conn.execute.assert_called_once()
        sql, agent_id, fired, threshold = pool._conn.execute.call_args[0]
    ...
    async def test_empty_fired_types_passes_none(self):
        ...
        with patch.object(db_module, "_pool", pool):
            await db_module.advance_clean_runs("agent-1", [])
        _, _, fired, _ = pool._conn.execute.call_args[0]
...
class TestWorkerIssueIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_upsert_called_when_signals_fired(self):
        ...
        with patch("detector_svc.worker.fetch_run_events", AsyncMock(return_value=[{"event_type": "run.started", "payload": {}}])), \
             ...
             patch("detector_svc.worker.RiskEngine", return_value=MagicMock(evaluate=MagicMock(return_value=MagicMock()))), \
        ...
        agent_id, fired = mock_upsert.call_args[0]
        self.assertEqual(agent_id, "agent-1")
    ...
    async def test_advance_called_even_when_no_signals(self):
        ...
        mock_advance.assert_called_once_with("agent-1", [])
...
class TestListIssues(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_empty_when_pool_is_none(self):
        with patch.object(db_module, "_pool", None):
            result = await db_module.list_issues("agent-1")
        self.assertEqual(result, [])

    async def test_returns_issues_with_correct_fields(self):
        pool = _make_pool()
        pool._conn.fetch = AsyncMock(return_value=[self._make_issue_row("open")])
        with patch.object(db_module, "_pool", pool):
            result = await db_module.list_issues("agent-1")
        self.assertEqual(len(result), 1)
        issue = result[0]
        self.assertEqual(issue["failure_type"], "TOOL_LOOP")
        self.assertEqual(issue["status"], "open")
        self.assertEqual(issue["affected_runs"], 5)
        self.assertIsInstance(issue["first_seen"], float)

    async def test_status_filter_appended_to_query(self):
        pool = _make_pool()
        pool._conn.fetch = AsyncMock(return_value=[])
        with patch.object(db_module, "_pool", pool):
            await db_module.list_issues("agent-1", status="resolved")
        call_args = pool._conn.fetch.call_args
        params = call_args[0][1:]
        self.assertIn("resolved", params)

    async def test_no_status_filter_queries_all(self):
        pool = _make_pool()
        pool._conn.fetch = AsyncMock(return_value=[])
        with patch.object(db_module, "_pool", pool):
            await db_module.list_issues("agent-1")
        params = pool._conn.fetch.call_args[0][1:]
        self.assertEqual(len(params), 1)  # only agent_id
